...
final class FinalResultTest extends TestCase
{
    ...
    // Array data comparation
    public function testReturnsTheCorrectHash(): void
    {
        $final = new FinalResult();
        $res = $final->results('tests/support/data_sample.csv');
        unset($res["document"]); 
        $this->assertEquals($res, $this->expected_return, "Error detected! documents are not equals.");
    }
}